...
def main():
    ...
    parser.add_argument("--use-keyboard", action="store_true", default=False, help="Use keyboard control")
    parser.add_argument(
        "--reset-delay",
        type=float,
        default=2.0,
    ...
    # gym_hil/PegInsertionGamepad-v0
    # gym_hil/PegInsertionKeyboard-v0
    # print("?")
    env = gym.make(
    "gym_hil/PegInsertionGamepad-v0",  # 使用新注册的环境 ID
    render_mode=args.render_mode,
    image_obs=True,
    use_gamepad=not args.use_keyboard,
    max_episode_steps=1000,  # 100 seconds * 10Hz
    ...
    try:
        while True:
            ...
            if terminated or truncated:
                ...
                print(f"最大 reward={max_reward:.2f}, ")
                max_reward = -np.inf  # 重置最大 reward     
                obs, _ = env.reset()
